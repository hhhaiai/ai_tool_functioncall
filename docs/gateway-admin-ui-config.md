...
- 云端默认拒绝下游参数中的服务端文件目标（例如 `path=/etc/passwd`、`file:///...`、`../...`）；只有管理员在对应 MCP server/tool 上显式设置 `allow_service_file_arguments: true` 时，才允许把这类路径交给 Gateway 服务侧 MCP。
...
- 云端默认拒绝 `localhost` / `127.0.0.1` / 私网 / link-local / multicast HTTP Action URL，并检查域名解析结果与 HTTP redirect 目标，避免下游通过 Gateway-owned action 间接访问服务端私网；只有管理员确认这是服务侧内部 action endpoint 时，才给单个 action 设置 `allow_private_network: true`。
- 通用 `WebFetch` / `WebSearch` 默认也拒绝服务端私网 URL、DNS 解析结果和 redirect；`image_generation` 的 provider URL 也走同一边界，并通过 `GATEWAY_IMAGE_MAX_DIMENSION` 限制下游请求尺寸。这类工具的私网访问只能由管理员通过 `gateway.allow_private_network_tools=true` 或 `GATEWAY_ALLOW_PRIVATE_NETWORK_TOOLS=1` 开启，不能由下游 tool arguments 开启。
