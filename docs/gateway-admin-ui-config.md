...
- text tool adapter compact token limit：弱上游文本工具适配前压缩 Claude Code/Codex 大 harness，默认 12000
...
2. 非默认当前目录的 `GATEWAY_WORKSPACE_ROOT` 环境变量。
3. Admin UI / 配置文件保存的 `gateway.workspace_root`。
4. 默认当前目录。

这样可以同时保证：启动脚本为了服务兜底导出的 `$PWD` 不会压过 UI 保存值；测试或运维显式设置到其他目录时仍能立即生效。
