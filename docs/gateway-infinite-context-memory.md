...
1. **SQLite 会话记忆**
   ...
   - 按 `session_key + workspace_root` 隔离，避免不同项目/会话串记忆。
...
- `computer_use`、`image_generation`、真实 UI 点击/键盘类工具仍需要本地插件/MCP/市场 connector；Gateway 会记录失败，方便后续增强。
...
- Memory tools: `Memory` / `SaveMemory` / `RecallMemory` / `remember`，真实读写 SQLite compact memory。
...
仍然不伪装成功的 connector 类工具：`computer_use`、`image_generation`、真实 UI click/type/press/scroll。它们需要 MCP/plugin connector，失败会进入 SQLite failures，后续由 MCP 市场补齐。
