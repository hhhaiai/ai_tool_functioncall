...
- Memory tools: `Memory` / `SaveMemory` / `RecallMemory` / `remember`，真实读写 SQLite compact memory；公开 direct tool 默认按当前认证 downstream client + 下游项目根隔离，不能通过 body 里的 `client_id` 或 `all_workspaces` / `include_all_workspaces` 枚举其它租户/项目。全局审计列表只走 admin API。
