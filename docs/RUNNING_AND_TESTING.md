...
| `gateway.max_log_payload_chars` | 200000 | 单个 request/response 日志 payload 截断上限；先遮盖敏感字段再截断 |
...
| `GATEWAY_MAX_LOG_PAYLOAD_CHARS` | gateway.max_log_payload_chars | 单个 request/response 日志 payload 字符上限，默认 200000 |
...
请求/响应日志和 Admin 配置展示会递归遮盖常见敏感字段，包括 `Authorization`、`X-API-Key`、`Cookie`、token、secret、password、`key_hash` 等；`must_change_password` 等非敏感状态字段会保留原值。请求/响应日志在遮盖之后会按 `gateway.max_log_payload_chars` / `GATEWAY_MAX_LOG_PAYLOAD_CHARS` 截断，避免长 prompt 或大响应导致 SQLite/JSONL 日志膨胀。
